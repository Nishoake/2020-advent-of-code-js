  ...
  for (let i = 0; i < passports.length; i++) {
    ...
    for (let [field, validator] of Object.entries(fields)) {
      // if (field !== 'cid') {
        ...
        if (hasField !== 1 || (withValidator && !validator(passportFields[field]))) {
          hasAllFields = false
        }
      // }
    }
...
const part1 = () => {
  ...
  console.log(`There are ${validatePassports(passports)} valid passports.`)
}

const part2 = () => {
  const passports = getInput()
  console.log(`Passports = ${passports}`)
  console.log(`There are ${validatePassports(passports, true)} valid passports.`)
}
